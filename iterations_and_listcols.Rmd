...
```{r}
map_dfr(list_normals, mean_and_sd, .id = "sample") # output a dataframe

map_dbl(list_normals, median) #output must be a single number
```
...
listcol_df = 
  tibble(
    name = c("a", "b", "c", "d"), #both have length 4, so can combine them
    sample = list_normals
  )
```
...
nsduh_df = 
  ...
  unnest(table) #expands the entire thing
```
...
weather_nest = 
  weather_df |> 
  nest(data = date:tmin) #collapse this
```
